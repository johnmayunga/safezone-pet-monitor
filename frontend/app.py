...
class PetTrackerApplication:
    ...
    def __init__(self):
        ...
        self.movement_history = defaultdict(lambda: deque(maxlen=100))
    ...
    def _processing_loop(self):
        """Main processing loop with corrected memory management."""
        frame_number = 0
        ...
        while self.running and not self.shutdown_event.is_set():
            try:
                ...
                # Add to processed frame queue with overflow management
                ...
                # CORRECTED: Periodic memory management
                if frame_number % 100 == 0:
                    # Clean up old photo references from video display
                    # if hasattr(self, 'video_display') and self.video_display:
                    #     if hasattr(self.video_display, 'current_photo'):
                    #         # Don't reassign, just clear old reference
                    #         pass
    ...
    def _update_display(self):
        ...
        if not self.processed_frame_queue.empty():
            try:
                ...
                # Queue size management - remove oldest if full
                if self.processed_frame_queue.full():
                    try:
                        self.processed_frame_queue.get_nowait()
                    except queue.Empty:
